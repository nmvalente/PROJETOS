...
public class Chunks
{
	...
	public void add(Message m){
		...
		String name = ip + File.separator + m.head.getFileId() + ".part" + m.head.getChunkNr() ;
	...
	public boolean remove(String address, String fileId, int chunkNr){
		if(hashmap.containsKey(address)){
			...
			for (Iterator<Chunk> it = hashmap.get(address).iterator(); it.hasNext();){
				...
				if (temp.getFileId().equals(fileId) && temp.getChunkNr() == chunkNr){
					removeChunk(address+File.separator+fileId+".part"+chunkNr);
					it.remove();
					return true;
				}
			}
		...
		if(hashmap.containsKey(address)){
			...
			for (Iterator<Chunk> it = hashmap.get(address).iterator(); it.hasNext();){
				...
				if (temp.getFileId().equals(fileId)){
					removeChunk(address+File.separator+fileId+".part"+temp.getChunkNr());
					it.remove();
				}
			}
			return true;
	...
	public Chunk find(String address, String fileId, int chunkNo){
		if(hashmap.containsKey(address)){
			...
			for (Iterator<Chunk> it = hashmap.get(address).iterator(); it.hasNext();){
				...
				if (temp.getFileId().equals(fileId) && temp.getChunkNr() == chunkNo ){
					return temp;
				}
			}
		}
	...
	public byte[] file(String address, Chunk c) throws IOException{
		File f = new File(address + File.separator + c.getFileId() + ".part" + c.getChunkNr());

		if (f.exists()){
			int fsize = (int) f.length();
			FileInputStream fis = new FileInputStream(address + File.separator + c.getFileId() + ".part" + c.getChunkNr());
			byte[] data = new byte[fsize];
			fis.read(data, 0, fsize);

			fis.close(); // fechei aqui
			return data;
		}
		return null;
	}
